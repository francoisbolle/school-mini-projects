...
void intro_rdv(vector <rdv *> *pT_rdv)
{
	...
	do
	{
		pT_rdv->resize(pT_rdv->size() + 1);			// obtention d'un pointeur supplémentaire
		(*pT_rdv)[pT_rdv->size() - 1] = new rdv();	// obtention d'une zone mémoire pour le dernier pointeur reçu
		cout << "\n\t * RDV No " << pT_rdv->size();
		cout << "\n\t\t==> No du dossier : ";
...
void lib_rdv(vector <rdv *> *pT_rdv, unsigned int indice)
{
	...
	{
		auto rdv_ToDel = pT_rdv->begin() + indice;	// capture de l'adresse de l'élément à supprimer au sein du container
		delete *rdv_ToDel._Ptr;		// libération de la zone mémoire en charge du pointeur (via pp)
		*rdv_ToDel._Ptr = 0;		// sécurisation du pointeur libéré
		pT_rdv->erase(rdv_ToDel);	// suppression du pointeur (dégagé du container)
	}
}

void lib_rdv(vector <rdv *> *pT_rdv)
{
	for (unsigned int x = 0; x < pT_rdv->size(); x++)
	{
		delete (*pT_rdv)[x];	// libération de la zone mémoire affectée au pointeur
		(*pT_rdv)[x] = 0;
	}
	pT_rdv->clear();	// suppression du tableau de pointeurs
}

void lib_rdv(vector <rdv *> &rT_rdv)
{
	for (unsigned int x = 0; x < rT_rdv.size(); x++)
	{
		delete rT_rdv[x];	// libération de la zone mémoire affectée au pointeur
		rT_rdv[x] = 0;
	}
	rT_rdv.clear();
}
