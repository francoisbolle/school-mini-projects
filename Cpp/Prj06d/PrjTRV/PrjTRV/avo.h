...
public:
	...
	unsigned int get_TprdvSize(void)
	{
		return this->Tprdv.size();
	}

	void add_rdv(rdv t_rdv)
	{
		this->Tprdv.resize(this->Tprdv.size() + 1);			// redimensionnement du tableau de pointeurs
		this->Tprdv[this->Tprdv.size() - 1] = new rdv(t_rdv);
	}

	void set_rdv(rdv t_rdv, unsigned int indice)
	{
		if (indice < this->Tprdv.size())*this->Tprdv[indice] = t_rdv;
		else throw Invalid_Value("Indice hors du tableau");
	}

	/*	FAILLE DE SECURISATION
	rdv* get_rdv(unsigned int indice)
	{
		return this->Tprdv[indice];
	}
	*/
	/*
	rdv get_rdv(unsigned int indice)
	...
	*/
	...
	rdv* get_rdv(unsigned int indice)
	{
		return new rdv(*this->Tprdv[indice]);
	}
};