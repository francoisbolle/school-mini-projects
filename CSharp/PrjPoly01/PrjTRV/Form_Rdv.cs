...
namespace PrjTRV
{
    public partial class Form_Rdv : Form
    {
        ...
        private void button_ValRdv_Click(object sender, EventArgs e)
        {
            try
            {
                switch (this.comboBox_TypRdv.SelectedIndex)
                { 
                    case 0:
                        this.inter_rdv = new rdv();     // ligature dynamique
                        break;

                    case 1:
                        this.inter_rdv = new rdvTrib();     // ligature dynamique
                        break;

                    case 2:
                        this.inter_rdv = new rdvPlaid();     // ligature dynamique
                        break;
                }
                ...
                //this.inter_rdv.set_TypRdv(this.comboBox_TypRdv.SelectedIndex); ==> PRIS EN CHARGE PAR LES CONSTRUCTEURS

                switch (this.comboBox_TypRdv.SelectedIndex)   // 0->Simple / 1->Trib / 2->Plaid
                {
                    case 1:
                        this.inter_rdv.set_Pstring(this.textBox_Ville.Text, true);    // Méthode POLY ==> set_Ville
                        this.inter_rdv.set_Pstring(this.textBox_Juge.Text, false);    // Méthode POLY ==> set_Juge
                        break;

                    case 2:
                        this.inter_rdv.set_Pstring(this.textBox_Ville.Text, true);    // Méthode POLY ==> set_Ville
                        this.inter_rdv.set_Pstring(this.textBox_Juge.Text, false);    // Méthode POLY ==> set_Juge
                        this.inter_rdv.set_Pbool(this.checkBox_Jury.Checked);         // Méthode POLY ==> set_Jury
                        break;
                ...
                this.avo_persist.add_rdvP(this.inter_rdv);
