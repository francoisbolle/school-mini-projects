...
namespace PrjTRV
{
    public partial class Form_Avo : Form
    {
        ...
        private void button_ValAvo_Click(object sender, EventArgs e)
        {
            ...
            //this.Tavo[this.Tavo.Length - 1] = new avo(ref inter_avo);
        ...
        private void refresh_dataGridView_RecapRdv(avo t_avo)
        {
            ...
            {
                t_jury = " - ";
                switch(t_avo.get_rdvP(x).get_TypRdv())
                {
                    case 0:
                        t_TypRdv = "Client";
                        break;
                    ...
                    case 2:
                        t_TypRdv = "Plaidoirie";
                        if(t_avo.get_rdvP(x).get_Pbool()) t_jury = "OUI";
                        else t_jury = "NON";
                        break;
                ...
                this.dataGridView_RecapRdv.Rows.Add(t_avo.get_rdvP(x).get_NoD().ToString(), t_TypRdv,
                                                            t_avo.get_rdvP(x).get_DatRdv(),
                                                               t_avo.get_rdvP(x).get_TimRdv(),
                                                                    t_avo.get_rdvP(x).get_Pstring(true),
                                                                        t_avo.get_rdvP(x).get_Pstring(false), t_jury);
            }
